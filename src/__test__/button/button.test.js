...
afterEach(cleanup);
it('renders okay', () => {
  render(<ButtonLoading loading={false} text={'Prueba'} />);
...
it('renders button correctly without loading', () => {
  render(<ButtonLoading loading={false} text={'Prueba'} />);
  expect(screen.getByTestId('button-test')).toHaveTextContent('Prueba');
});


it('renders button correctly with loading', () => {
  render(<ButtonLoading loading={true} text={'Prueba'} />);
  expect(screen.getByTestId('button-test')).not.toHaveTextContent('Prueba');
});


it('renders button correctly disabled', () => {
  render(<ButtonLoading disabled={true} loading={false} text={'Prueba'} />);
  expect(screen.getByTestId('button-test')).toHaveAttribute('disabled');
});

it('renders button correctly enabled', () => {
  render(<ButtonLoading disabled={false} loading={false} text={'Prueba'} />);
  expect(screen.getByTestId('button-test')).not.toHaveAttribute('disabled');
});


it('renders loading', () => {
  render(<ButtonLoading loading={true} disabled={false} text={'Prueba'} />);
